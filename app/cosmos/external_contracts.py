...
async def junoswap_locks(session):
    staking_contracts = await make_get(session, 'https://raw.githubusercontent.com/CosmosContracts/junoswap-asset-list/main/token_list.json')
    ...
    stake_by_swap = {x['swap_address'] : { 'staking_address' : x['staking_address'], 'rewards' : [] } for x in s['tokens'] if 'staking_address' in x}
