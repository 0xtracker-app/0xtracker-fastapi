...
async def lookup_token_info(token_list, network):
        ...
        for each in token_list:
            if f'{each}_decimal' in missing_tokens_info and f'{each}_symbol' in missing_tokens_info:
                ...
                format_token_data[each] = {'tkn0d' : missing_tokens_info[token_decimal], 'tkn0s' : missing_tokens_info[token_symbol]}
            else:
...
async def scan_ethlogs_approval(network, address, session, mongodb):
    ...
    latest_block = 'latest' ##WEB3_NETWORKS[network]['connection'].eth.block_number
    ...
    for each in data:
        ...
        if each['address'] not in format_approvals['approvals']:
            ...
            format_approvals['approvals'][token_approved] = {'tokenID': token_approved, 'tokenData': token_data,'network' : network, 'contracts' : {contract_approved : {'tx': [{'contractApproved' : contract_approved, 'amount' : amount, 'blockNumber' : block_number, 'tx' : each['transactionHash']}]}}}
            
        else:
            if contract_approved not in format_approvals['approvals'][token_approved]['contracts']:
                format_approvals['approvals'][token_approved]['contracts'][contract_approved] = {'tx' : [{'contractApproved' : contract_approved, 'amount' : amount, 'block_number' : block_number, 'tx' : each['transactionHash']}]}
            else:
