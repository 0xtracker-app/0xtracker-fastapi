...
async def get_evm_positions(wallet, farm_id, mongo_db, http_session):
    ...
    if farm_configuraiton['network'] in INCH_SUPPORTED:
        prices = await oracles.get_one_inch_quote(token_list_from_stakes(build_meta_data, farm_configuraiton), http_session)
    elif farm_configuraiton['network'] == 'avax':
        prices = await oracles.avax_router_prices(token_list_from_stakes(build_meta_data, farm_configuraiton), ['PNG', 'JOE', 'SUSHI'])
    elif farm_configuraiton['network'] == 'ftm':
        prices = await oracles.fantom_router_prices(token_list_from_stakes(build_meta_data, farm_configuraiton), ['SPOOKY', 'HYPER', 'SPIRIT', 'WAKA', 'PAINT'])
    elif farm_configuraiton['network'] == 'oke':
        prices = await oracles.oke_router_prices(token_list_from_stakes(build_meta_data, farm_configuraiton), ['PANDA', 'CHERRY', 'KSWAP'])
    elif farm_configuraiton['network'] == 'kcc':
        prices = await oracles.kcc_router_prices(token_list_from_stakes(build_meta_data, farm_configuraiton), ['KUSWAP', 'KOFFEE', 'KANDY', 'BONE'])
